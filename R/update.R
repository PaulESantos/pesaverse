#' Update pesaverse packages
#'
#' This will check to see if all pesaverse packages (and optionally,
#' their dependencies) are up-to-date, and will install after an
#' interactive confirmation.
#'
#' @inheritParams pesaverse_deps
#' @export
#' @examples
#' \dontrun{
#' pesaverse_update()
#' }
pesaverse_update <- function(recursive = FALSE, repos = getOption("repos")) {

  deps <- pesaverse_deps(recursive, repos)
  behind <- poorman::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All pesaverse packages up-to-date")
    return(invisible())
  }
...
#' Get a situation report on the pesaverse
#'
#' This function gives a quick overview of the versions of R and
#' RStudio as well as all pesaverse packages. It's primarily designed
#' to help you get a quick idea of what's going on when you're helping
#' someone else debug a problem.
#'
#' @export
pesaverse_sitrep <- function() {
...
#' List all pesaverse dependencies
#'
...
#' @export
pesaverse_deps <- function(recursive = FALSE, repos = getOption("repos")) {
  pkgs <- utils::available.packages(repos = repos)
  deps <- tools::package_dependencies("pesaverse", pkgs,
                                      recursive = recursive)
  ...
  local_version <- lapply(pkg_deps, pkgVersion)
